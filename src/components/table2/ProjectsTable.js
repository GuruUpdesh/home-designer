	...
	function searchHandler(value) {
		...
		for (let i = 0; i < Object.keys(props.tableContent.tableData[0]).length; i++) {
			const result = props.tableContent.tableData.filter((object) => {
                console.log(Object.keys(props.tableContent.tableData[0])[i])
				return (
							...
							return (
								<TableRow
									data={[
										...
										{ value: data.dateStarted, type: "text" },
										{ value: data.complete, type: "checkbox" },
                    { value: data.description, type: "text" },
										// { value: data.phone, type: "text" },
										{ value: data.billingHours, type: "list" },
									]}
									index={index}
